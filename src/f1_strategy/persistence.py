...
class DuckDBPersistence:
    ...
    def run_summaries(self, limit: int = 12) -> list[dict]:
        ...
        for session_id, car_id, recorded_at_ms, payload_json in strategies:
        ...
        for session_id, car_id, lap, recorded_at_ms, latency_ms, payload_json in rows:
            key = (session_id, car_id)
            payload = json.loads(payload_json)
            ...
            summary["max_tire_wear_pct"] = max(
                summary["max_tire_wear_pct"],
                float(payload["tire_wear_pct"]),
            )
            ...
            interval_width = float(payload["uncertainty_high_s"]) - float(
                payload["uncertainty_low_s"]
            )
